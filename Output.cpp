...
int Output::GetCellStartX(const CellPosition& cellPos) const
{
	...
	int startx = cellPos.HCell() * UI.CellWidth;
...
{
	///TODO: implement the following function as described in Output.h file
	int starty = cellPos.VCell() * UI.CellHeight + UI.ToolBarHeight;
...
void Output::ClearToolBarWriting() const
{
	// Clear by drawing a rectangle filled with toolbar background color
	pWind->SetPen(UI.ToolBarColor, 1);
	pWind->SetBrush(UI.ToolBarColor);
	pWind->DrawRectangle(UI.MenuItemWidth * 5, 0, UI.width, UI.ToolBarHeight);
...
void Output::CreateDesignModeToolBar() const
{
	...
	///TODO:(((DONE BY AKRAM))) Prepare images for each menu item and add it to the list
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Input_Dice_Value.jpg";//You can change the image
...
void Output::PrintPlayersInfo(string info)
{
	...
	ClearToolBarWriting();//new function that doesn't delete the whole toolbar
	...
	pWind->GetStringSize(w, h, info);//not SURE-akram
	...
	pWind->DrawString(x, y, info);//akram
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	///TODO:(((DONE-AKRAM))) Validate the playerNum, if not valid return
	...
	//AKRAM-NOT SURE AT ALL
	pWind->SetPen(playerColor);
	pWind->SetBrush(playerColor);
	pWind->DrawCircle(x, y, radius, FILLED);//not SURE-AKRAM
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	///TODO:(((DONE))) Validate the Cell Position (Must be Vertical Cells AND toCell below fromCell, otherwise, Do NOT draw)
	...
	int lineNumbers = abs(toCell.VCell() - fromCell.VCell());
	for (int i = 0; i < lineNumbers; i++)
	{
		if (fromStartY<toStartY)
			pWind->DrawLine(x12, fromStartY + (i + 1) * UI.CellHeight, x34, fromStartY + (i + 1) * UI.CellHeight, FRAME);
		else
			pWind->DrawLine(x12, toStartY + (i + 1) * UI.CellHeight, x34, toStartY + (i + 1) * UI.CellHeight, FRAME);
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int x[4] = { cellStartX,x12,2 * x12 - cellStartX,x12 }; int y[4] = { y1,y1 - (fromStartY - y1) / 2,y1,y1 + (fromStartY - y1) / 2 };//should be edited-Akram-Ayman
...
/*
AKRAM
PrintPlayersInfo(); should be revised
DrawPlayer(); I don't know how to make the color and it needs to be revised
anywhere I use pWind, I don't know if I have to write setPen or not or setBrush!!!
*/