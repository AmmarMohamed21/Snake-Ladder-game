#include "CardTen.h"

CardTen::CardTen(const CellPosition& pos) : Card(pos) // set the cell position of the card
{
	cardNumber = 10; // set the inherited cardNumber data member with the card number
}

CardTen::~CardTen(void)
{
}

void CardTen::ReadCardParameters(Grid* pGrid)
{
	
	Output* pOut = pGrid->GetOutput();
	Input* pIn = pGrid->GetInput();
	if (pGrid->GetCard(10))
	{
		SetParameters(pGrid->GetCard(10)->GetCardParameters());
		return;
	}
	int input1 = 0;
	while (input1 <= 0)
	{ 
		pOut->PrintMessage("CardTen: Enter its Card price ...");
		input1 = pIn->GetInteger(pOut);
	}
	Card_price = input1;
	
	int input2 = 0;
	while (input2 <= 0)
	{
		pOut->PrintMessage("CardTen: Enter its fees to pay  ...");
		input2 = pIn->GetInteger(pOut);
	}
	Fees_to_pay = input2;

	pOut->ClearStatusBar();
}

void CardTen::Apply(Grid* pGrid, Player* pPlayer)
{

	///TODO: Implement this function as mentioned in the guideline steps (numbered below) below
// == Here are some guideline steps (numbered below) (numbered below) to implement this function ==

	// 1- Call Apply() of the base class Card to print the message that you reached this card number
	
	Card::Apply(pGrid, pPlayer);

	Output* pOut = pGrid->GetOutput();
	
	Input* pIn = pGrid->GetInput();
	
	if (pGrid->GetCardOwner(10))
	{
		if(pPlayer != pGrid->GetCardOwner(10))
		{
			pGrid->PrintErrorMessage("You have reached a bought station, Click to continue...");
			pPlayer->SetWallet(pPlayer->GetWallet()-Fees_to_pay);
			pGrid->GetCardOwner(10)->SetWallet(pGrid->GetCardOwner(10)->GetWallet()+Fees_to_pay);
		}
	}
	else
	{
	    pOut->PrintMessage("You have reached a station. Do you want to buy it? y/n");	
	
	    char ans =  pIn->GetString(pOut)[0];
		pOut->ClearStatusBar();
	    if (ans == 'y' || ans == 'Y')
	    {
	       if(pPlayer->GetWallet()>Card_price)
	       {
		       pPlayer->SetWallet(pPlayer->GetWallet()-Card_price); 
		       int* cards = pPlayer->GetCardsOwned();
		       cards[0] = 10;
		       pPlayer->SetCardsOwned(cards);
	       }
	
		}
	}

}

int* CardTen::GetCardParameters()
{
	int* pars = &Card_price;
	pars[1] = Fees_to_pay;
	return pars;
}

void CardTen::SetParameters(int* pars)
{
	Card_price = pars[0];
	Fees_to_pay = pars[1];
}